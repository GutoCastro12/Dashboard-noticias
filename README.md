[README.md](https://github.com/user-attachments/files/30332105/README.md)
...
`fetch_cvm_fatos` varre o dataset IPE inteiro e casa por alias contra toda a
watchlist — ou seja, **não depende de flag no cadastro**. Toda companhia aberta
ou fundo registrado já tem cobertura oficial automática. O que não se sabia é
quantos dos emissores **brasileiros não listados** (PE, crédito privado, SPE)
são de fato filiantes na CVM. O comando responde isso com dado: cruza a
watchlist com todos os protocolos do ano e classifica cada emissor em
`filiante_cvm`, `nao_filiante`, `esperado_filiante_sem_protocolo_no_ano` ou
`nao_aplicavel_veiculo`. Saída: `auditoria_cobertura_cvm.csv`.
