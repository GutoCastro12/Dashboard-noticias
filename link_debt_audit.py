...
# ═══════════════════════════════════════════════════════════════════════
# RESOLVEDOR CANÔNICO DE TOKENS DO GOOGLE NEWS (batchexecute)
# ═══════════════════════════════════════════════════════════════════════
#
# Consolidação de duas implementações que haviam divergido:
#   - risk_dashboard.py: _resolve_one_gnews / _gnews_batchexecute — resolvedor
#     FORTE (decode inline + batchexecute + redirect), usado só para fontes
#     PRINCIPAIS via resolve_history_urls/resolve_google_news_urls;
#   - link_debt_audit.py: resolve_article_url — resolvedor FRACO (só extração
#     de querystring + redirect HTTP simples), usado para corroboradoras E
#     pelo --repair-links-only inteiro.
# Os tokens modernos do Google News (formato /rss/articles/{token} sem URL
# embutida) só cedem ao batchexecute. Por isso 357-373 corroboradoras
# ficavam presas em redirect_nao_resolvido: elas nunca passavam pelo
# resolvedor forte. HTTP 200 do endpoint do Google News != URL final da
# notícia — o corpo da resposta ainda referencia news.google.com.
#
# Esta é agora a ÚNICA implementação. Usada por TODOS os caminhos: fonte
# principal, corroboradora, --repair-links-only, workflow e coleta futura.

GNEWS_BATCHEXECUTE_RPC = "Fbv4je"
GNEWS_HOSTS = ("news.google.com",)


def _is_gnews_host(host: str) -> bool:
    return any(h in (host or "") for h in GNEWS_HOSTS)


def gnews_article_id(gurl: str) -> str:
    """Extrai o token do artigo de uma URL /articles/ ou /rss/articles/."""
    try:
        return gurl.split("/articles/")[1].split("?")[0]
    except Exception:
        return ""


def gnews_inline_decode(gurl: str) -> tuple[str, str]:
    """Tenta decodificar o token base64 SEM rede. Funciona nos tokens antigos
    que embutem a URL do veículo diretamente no protobuf. Devolve
    (url_ou_vazio, motivo_se_falhou)."""
    art_id = gnews_article_id(gurl)
    if not art_id:
    ...
    try:
        ...
        return "", f"base64_invalido:{type(exc).__name__}"
    m = re.search(rb"https?://[\w\-./%?=&#:+,~]+", raw)
    if not m:
        return "", "sem_url_embutida"
    cand = m.group(0).decode("utf-8", "ignore")
    if "google.com" in cand or len(cand) <= 15:
        return "", "candidato_invalido"
    return cand, ""


def gnews_batchexecute(gurl: str, session, *, timeout: int = 15) -> tuple[str, str]:
    """Resolve o token via o endpoint interno batchexecute do Google News.

    Protocolo (documentado publicamente, ex. projeto googlenewsdecoder):
      1. busca a página do artigo (/articles/{id} ou /rss/articles/{id}) e
         extrai `data-n-a-sg` (assinatura) e `data-n-a-ts` (timestamp);
      2. POST em .../_/DotsSplashUi/data/batchexecute?rpcids=Fbv4je com esses
         parâmetros no corpo `f.req`;
      3. a resposta (formato wrb.fr) contém a URL final do veículo.

    Devolve (url_ou_vazio, motivo_se_falhou)."""
    art_id = gnews_article_id(gurl)
    if not art_id:
        return "", "token_ausente"
    params = None
    for base in ("https://news.google.com/articles/",
        ...
        try:
            page = session.get(base + art_id, timeout=timeout,
            ...
            ultimo_erro = f"{type(exc).__name__}: {str(exc)[:100]}"
            continue
    if not params:
        return "", "parametros_sg_ts_nao_encontrados"
    ...
    try:
        ...
        return "", f"post_falhou:{type(exc).__name__}: {str(exc)[:100]}"
    if resp.status_code != 200:
        return "", f"http_{resp.status_code}_no_batchexecute"
    return parse_batchexecute_response(resp.text)


def parse_batchexecute_response(text: str) -> tuple[str, str]:
    """Parser isolado (testável sem rede) da resposta batchexecute.

    O corpo vem prefixado com `)]}'` e contém JSON escapado dentro de outro
    JSON. Procura o padrão \"garturlres\",\"<url>\" e, na ausência dele,
    cai para a primeira URL http(s) que não seja do próprio Google."""
    ...
    if not m:
        return "", "url_nao_encontrada_na_resposta"
    try:
        url = m.group(1).encode().decode("unicode_escape")
    except Exception:
        url = m.group(1)
    if "google.com" in _host(url):
        return "", "resposta_ainda_aponta_para_google"
    return url, ""


def gnews_redirect_fallback(gurl: str, session, *, timeout: int = 15) -> tuple[str, str]:
    """Último recurso: segue o redirect HTTP e, falhando, procura o atributo
    `data-n-au` (URL do artigo) embutido na página. NUNCA aceita como
    resolvido um destino cujo host ainda seja do Google — HTTP 200 do Google
    News não é URL final."""
    try:
        r = session.get(gurl, timeout=timeout, allow_redirects=True,
                        headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; "
                                               "Win64; x64)"})
    except Exception as exc:  # noqa: BLE001
        return "", f"{type(exc).__name__}: {str(exc)[:100]}"
    final = r.url or ""
    if final and not _is_gnews_host(_host(final)) and "google.com" not in final:
        return final, ""
    m = re.search(r'data-n-au="([^"]+)"', r.text)
    if m and "google.com" not in m.group(1):
        return m.group(1), ""
    return "", "destino_final_ainda_e_google"


def resolve_gnews_token(gurl: str, *, session=None, cache: dict | None = None,
                        allow_network: bool = True) -> dict:
    """Resolvedor CANÔNICO de um token do Google News, com cadeia de métodos
    em ordem de custo crescente. Único caminho usado por toda a aplicação.

    Ordem: cache → decode inline (sem rede) → batchexecute → redirect HTTP
    (com leitura de data-n-au). Cada etapa só roda se a anterior falhar.
    Devolve {"url": str, "method": str, "error": str}."""
    cache = cache if cache is not None else {}
    ent = cache.get(gurl)
    if isinstance(ent, dict) and ent.get("url") and ent.get("exact"):
        if not _is_gnews_host(_host(ent["url"])):
            return {"url": ent["url"], "method": "cache", "error": ""}

    url, err = gnews_inline_decode(gurl)
    if url:
        return {"url": url, "method": "inline_decode", "error": ""}
    erros = [f"inline_decode:{err}"]

    if not allow_network:
        return {"url": "", "method": "offline",
                "error": "resolução exige rede (rodar no GitHub Actions)"}

    import requests
    s = session or requests.Session()

    url, err = gnews_batchexecute(gurl, s)
    if url:
        return {"url": url, "method": "batchexecute", "error": ""}
    erros.append(f"batchexecute:{err}")

    url, err = gnews_redirect_fallback(gurl, s)
    if url:
        return {"url": url, "method": "redirect_http_data_n_au", "error": ""}
    erros.append(f"redirect_http:{err}")

    return {"url": "", "method": "nenhum", "error": "; ".join(erros)[:300]}
...
def resolve_article_url(original_url: str, *, domain: str = "",
    ...
    if not is_redirector(u0):
        ...
        if alvo and is_direct_valid(alvo):
            ...
            r["last_checked_at"] = datetime.now(timezone.utc).isoformat()
            return r
        # 4) token do Google News: cadeia canônica (cache → inline_decode →
        #    batchexecute → redirect+data-n-au). É a MESMA função usada por
        #    fonte principal, corroboradora e --repair-links-only.
        if _is_gnews_host(r["original_host"]):
            gres = resolve_gnews_token(u0, session=session, cache=cache,
                                       allow_network=allow_network)
            alvo = gres["url"]
            # HTTP 200 do Google News não é URL final: exige host diferente
            # do agregador e estrutura de URL direta válida.
            if alvo and not _is_gnews_host(_host(alvo)) and is_direct_valid(alvo):
                can = canonicalize(alvo)
                r.update(resolved_url=alvo, canonical_url=can, display_url=can,
                         final_host=_host(can), link_health="redirect_resolvido",
                         resolution_method=gres["method"])
                r["last_checked_at"] = datetime.now(timezone.utc).isoformat()
                return r
            r.update(link_health=("nao_verificado" if gres["method"] == "offline"
                                  else "redirect_nao_resolvido"),
                     resolution_method=gres["method"],
                     resolution_error=gres["error"] or "token não resolvido")
        # 5) rede: fallback genérico para redirecionadores que NÃO são do
        #    Google News (bit.ly, t.co, encurtadores diversos)
        elif allow_network:
            try:
                import requests
                s = session or requests.Session()
                ...
                final = resp.url or ""
                if is_direct_valid(final) and not _is_gnews_host(_host(final)):
