...
def aggregate_scores(history: dict, cfg: dict, window_days: int | None = None) -> list[dict]:
    """Score por emissor no Radar: soma dos scores dos tipos de evento distintos
    na janela (peso-base × confiança da fonte de maior peso) + bônus limitado por
    fontes independentes (corroboração). Republicações do mesmo evento NÃO
    inflam o score — o número de notícias não altera a pontuação nem a ordenação.
    Alinhado à mesma lógica de corroboração da aba Evolução."""
    taxonomy = {e["id"]: e for e in cfg["taxonomy"]}
    scoring = cfg["scoring"]
    if window_days is None:
    ...
    for company, entry in per_company.items():
        ...
        for eid, recs in entry["events"].items():
            ...
            # peso-base do evento pela ocorrência de maior confiança (não por
            # contagem de notícias). Republicações NÃO inflam o score.
            trust_w = max(trust_of_rec(r, cfg)[1] for r in recs)
            base = round(ev["score"] * trust_w)
            # bônus por FONTES independentes (domínios distintos + corroborações
            # persistidas), limitado e decrescente — mesma lógica da Evolução.
            domains = {r.get("domain", "") for r in recs if r.get("domain")}
            for r in recs:
                domains |= {c.get("domain", "") for c in r.get("corrob_sources", [])
                            if c.get("domain")}
            n_extra = max(0, len(domains) - 1)
            corrob_bonus = sum(bonus_steps[i] if i < len(bonus_steps) else 0
                               for i in range(n_extra))
            score += base + corrob_bonus
            n_sources = max(len(domains), 1 + sum(len(r.get("corrob_sources", []))
                                                  for r in recs))
            if SEVERITY_ORDER[ev["severity"]] < SEVERITY_ORDER[worst]:
                worst = ev["severity"]
            event_chips.append({
                "id": eid, "label": ev["label"], "severity": ev["severity"],
                "score": ev["score"], "count": len(recs), "sources": n_sources,
            })
