...
def build_company_query(company: dict, taxonomy: list[dict]) -> str:
    """Consulta ampla por emissor: nome + termos de risco mais fortes."""
...
def dedupe_articles(articles: list[dict], history: dict, cfg: dict) -> list[dict]:
    ...
    if not dd.get("enabled", True):
        return articles
    threshold = dd.get("similarity", 0.80)
...
def validate_critical_events(articles: list[dict], cfg: dict, history: dict) -> None:
    ...
    if not api_key:
        print(" ⚠️  Validação LLM habilitada, mas sem GEMINI_API_KEY. Pulando.")
        return
    ...
    pending = [
        ...
        and any(e["severity"] == "critico" for e in a.get("events", []))
    ]
    if not pending:
        return
    ...
    print(f" 🤖 Validando {len(pending)} evento(s) crítico(s) via Gemini…")

    for art in pending:
        criticals = [e for e in art["events"] if e["severity"] == "critico"]
        others = [e for e in art["events"] if e["severity"] != "critico"]
        ev_desc = "\n".join(f"- {e['id']}: {e['label']}" for e in criticals)
        prompt = (
            "Você é um analista de risco de crédito. A manchete abaixo foi "
            "classificada por keywords nos eventos críticos listados. CONFIRME "
            "apenas os eventos que de fato OCORRERAM com a(s) empresa(s) "
            f"{', '.join(art['companies'])} — descarte especulação ('pode ser "
            "rebaixada'), negação ('afasta risco de'), condicional, contexto "
            "histórico ou evento de terceiros. Responda APENAS JSON: "
            '{"confirmed_event_ids": ["..."]}.\n\n'
        ...
        try:
            ...
            confirmed = set(json.loads(resp.text).get("confirmed_event_ids", []))
            kept_criticals = [e for e in criticals if e["id"] in confirmed]
            dropped = [e["label"] for e in criticals if e["id"] not in confirmed]
            if dropped:
...
def build_evolution(history: dict, cfg: dict) -> list[dict]:
    ...
    tiers = {c["name"]: c.get("tier", 2) for c in cfg.get("watchlist", [])}
...
def aggregate_scores(history: dict, cfg: dict) -> list[dict]:
    ...
    taxonomy = {e["id"]: e for e in cfg["taxonomy"]}
    scoring = cfg["scoring"]
    window_days = cfg["dashboard"].get("window_days", 7)
    ...
    for company, entry in per_company.items():
        ...
        ranking.append({
            ...
            "attention": score >= scoring.get("attention_threshold", 80),
        })
...
def build_feed(history: dict, cfg: dict) -> list[dict]:
...
def render_html(ranking: list[dict], evolution: list[dict], feed: list[dict],
                cfg: dict, demo: bool) -> str:
    template_path = Path(__file__).parent / "template_risco.html.j2"
    with open(template_path, "r", encoding="utf-8") as f:
        template = Template(f.read())

    window_days = cfg["dashboard"].get("window_days", 7)
    counts = {"critico": 0, "alto": 0, "medio": 0, "info": 0}
    for art in feed:
        counts[art["severity"]] += 1

    companies = sorted({c for art in feed for c in art.get("companies", [])})
    status_counts = {"critico": 0, "atencao": 0, "monitorar": 0}
    ...
    return template.render(
        ...
        evolution_days=cfg.get("evolution", {}).get("window_days", 90),
        ranking=ranking,
        evolution=evolution,
...
def main():
    ...
    ranking = aggregate_scores(history, cfg)
    evolution = build_evolution(history, cfg)
    feed = build_feed(history, cfg)

    if ranking:
        print("\n 📊 Radar semanal (janela de "
              f"{cfg['dashboard'].get('window_days', 7)} dias):")
        for r in ranking[:5]:
            flag = " ⚠️ ATENÇÃO" if r["attention"] else ""
            print(f"   {r['score']:>4} pts — {r['company']}{flag}")
    if evolution:
        print(f"\n 📈 Evolução ({cfg.get('evolution', {}).get('window_days', 90)} dias):")
    ...
    # 4) Render
    html = render_html(ranking, evolution, feed, cfg, demo=args.demo)
