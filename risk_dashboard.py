...
def fetch_all(cfg: dict, run_count: int = 0) -> list[dict]:
    ...
    for company in active:
        ...
        _hl, _gl = locale_for_company(company, cfg)
        _tel = _SEARCH_TELEMETRY.setdefault(company["name"], {
            "searched": True, "locale": f"{_hl}/{_gl}", "queries": 0,
            "http_success": 0, "raw_articles": 0, "error": ""})
        _tel["queries"] += 1
        _res = fetch_query(q, cfg, session, (_hl, _gl))
        _tel["http_success"] += 1 if _res is not None else 0
        _tel["raw_articles"] += len(_res or [])
        for art in (_res or []):
...
def audit_international_coverage(cfg: dict, history_path: str = "risk_history.json",
    ...
    for c in est:
        ...
        if not tem_tel:
            ...
            merro = tel.get("error", "") or ""
            mstatus = ("erro" if merro else
                       ("respondeu_sem_resultado" if not tel.get("raw_articles") else "ok"))
        off = c.get("official") or {}
        edgar = edgar_eligible(c)
        ...
        if edgar:
            modo = "oficial ativa (SEC/EDGAR)"
        elif ri_rss:
            modo = "oficial ativa (RSS do RI)"
        elif ri_news:
            modo = "oficial ativa (página de RI)"
        ...
        if not pesquisado:
            ...
            motivo, status = ("elegível, mas SEM evidência de execução "
                              "(run_meta ausente) — não afirmar 'pesquisado'"), "SEM_EVIDENCIA"
        elif merro:
            motivo, status = f"consulta com ERRO: {merro[:60]}", "ERRO_FONTE"
        elif not tel.get("raw_articles"):
            motivo, status = "consulta executada, 0 resultados brutos", "ZERO_BRUTO"
        elif n90 == 0:
            motivo, status = ("resultado bruto coletado, mas nada atribuído ao emissor "
                              "(alias/idioma a revisar)"), "SEM_ATRIBUICAO"
        elif n90 == 0 and alias_curto:
            motivo, status = "pesquisado, 0 resultados — alias curto/ambíguo suspeito", "ATENÇÃO"
        elif n90 == 0:
            motivo, status = "pesquisado, 0 resultados no período", "ATENÇÃO"
        elif n7 == 0:
        ...
        linhas.append({
            ...
            "midia_respondeu_ultima_execucao": ("sim" if tem_tel and tel.get("http_success")
            ...
            "consultas_com_sucesso": tel.get("http_success", "") if tem_tel else "",
            "consultas_com_erro": (tel.get("queries", 0) - tel.get("http_success", 0)) if tem_tel else "",
            ...
            "edgar_execucao_evidenciada": ("sim" if oficiais_90d else
                                           ("sem_evidencia" if edgar else "n/a")),
            "filings_encontrados_90d": oficiais_90d,
            "ri_rss_ativo": "sim" if ri_rss else "não",
            "ri_news_ativo": "sim" if ri_news else "não",
    ...
    for l in linhas:
        ...
        p["oficial_ativa"] += l["modo_cobertura"].startswith("oficial ativa")
    ...
    _write_rows_csv(os.path.join(outdir, "cobertura_internacional_por_pais.csv"), rows_pais,
                    ["pais", "monitorados", "elegiveis", "exec_evidenciada", "com_noticia_90d",
                     "oficial_ativa", "so_documentada", "apenas_midia", "zero_resultados"])
    ...
    prio = []
    for l in linhas:
        if l["modo_cobertura"].startswith("oficial ativa"):
            continue
        try:
        ...
        prio.append({"emissor": l["emissor"], "pais": l["pais"],
                     ...
                     "lacuna": ("sem EDGAR e sem RI" if l["ri_news_ativo"] == "não"
                                else "RI a validar"),
                     "candidato_fonte": ("RI do emissor (a_confirmar)"),
                     "dificuldade": "baixa" if l["ri_news_ativo"] == "sim" else "média",
                     "prioridade": p,
    ...
    of_ativa = sum(1 for l in linhas if l["modo_cobertura"].startswith("oficial ativa"))
...
def main():
    ...
    _e4 = (args.analyze_audit_cvm or args.compare_audit_cvm or args.analyze_probe_sources
           ...
           or args.audit_international_coverage)
    if _e4:
    ...
    for art in articles:
        ...
        # 4H.0 — resolução por família ANTES de gravar no histórico: o que é
        # secundário (outlook de uma ação de rebaixamento) não vira ocorrência,
        # não pontua e não gera pontinho — só metadado exibido no card.
        _prin, _sec, _mot = resolve_event_families(_evs, cfg, art.get("title", ""))
        art["events"] = _prin
        if _sec:
    ...
    try:
        ...
        _rm["run_finished_at"] = datetime.now(timezone.utc).isoformat()
        with open("run_meta.json", "w", encoding="utf-8") as _f:
