...
def _match_ipe(c: dict, idx: dict, strong: dict | None) -> tuple:
    ...
    if strong and strong.get("tipo") == "ambiguo":
...
def analyze_audit_cvm(new_csv, old_csv=None, outdir="."):
    ...
    rev_rows = [{"emissor": r.get("emissor", ""), "tipo_match": r.get("tipo_match", ""),
                 "confianca": r.get("confianca_match", ""), "candidatos": r.get("candidatos", ""),
                 "motivo": r.get("motivo_decisao", ""),
                 "acao": "definir codigo_cvm/cnpj/razao_social no config e reauditar"}
                for r in rows if (r.get("status") or "") == "revisar"]
    _write_rows_csv(os.path.join(outdir, "revisar_identificadores.csv"), rev_rows,
                    ["emissor", "tipo_match", "confianca", "candidatos", "motivo", "acao"])
    return {"status": "ok", "recomendacao": rec, "bloqueios": len(bloq), "warnings": len(warn),
            "revisar": len(revisar), "contagem": dict(cont)}
...
def _probe_categoria(r):
    ...
    if "html estático" in txt or ("html" in ct and "js" not in txt and "dinam" not in txt):
        return "HTML estático", "candidato — preparar coletor atrás de flag"
...
def analyze_probe_sources(probe_csv, outdir="."):
    ...
    def decide_alvo(nome):
        ...
        c = d["categoria"]
        if c in _BONS:
            return f"{nome}: {c} → candidato; preparar coletor atrás de flag (após re-probe estável)"
        if c == "timeout":
    ...
    if fnet_ok:
        proxima[3] = "4. FNET respondeu estável no re-probe → **avaliar coletor FNET atrás de flag**."
    ...
    L += ["", "## 4. FNET — situação",
          f"- FNET: **{fnet_cat}** → {'utilizável' if fnet_ok else 'NÃO utilizável ainda (re-probe)'}.",
          "  *Não confundir com CVM FIDC/B3 (HTML estático): são fontes diferentes.*"]
...
def quality_gate(cfg, audit_csv=None, probe_csv=None, coverage_backlog=None,
    ...
    if prb is None:
        ...
        W(any("anti-bot" in (r.get("veredito") or "").lower() for r in prb), "fonte(s) LatAm anti-bot")
        fnet = next((r for r in prb if "fnet" in (r.get("url") or "").lower()), None)
        if fnet is not None:
            fv = (fnet.get("veredito") or "").lower() + (fnet.get("erro") or "").lower()
            fnet_ruim = ("timeout" in fv or "indispon" in fv or "404" in fv)
            W(fnet_ruim, "FNET timeout/indisponível — NÃO tratar como fonte utilizável; re-probe antes de qualquer coletor")
        # rastreabilidade: nº de alvos medidos
        W(len(prb) < 15, f"probe com poucos alvos ({len(prb)}) — conferir se o CSV tem todos (esperado ~22)")
...
def post_actions_report(cfg, audit_csv=None, probe_csv=None, coverage_backlog=None,
    ...
    L = ["# Relatório pós-Actions — Radar de Risco", "",
         ...
         (f"Ver `relatorio_probe_fontes.md` ({prb.get('n_alvos','?')} alvos). "
          f"FNET: **{prb.get('fnet_categoria','?')}** ({'utilizável' if prb.get('fnet_ok') else 'NÃO utilizável — re-probe'})."
