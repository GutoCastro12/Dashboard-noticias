[README.md](https://github.com/user-attachments/files/29869061/README.md)
...
- **Validação LLM dos críticos** (`llm.enabled` + `validate_critical`, **ligada por padrão**): eventos críticos detectados por keyword são confirmados pelo Gemini antes de pontuar — descarta especulação, negação, casos antigos reaquecidos, e agora também confirma **de qual empresa** é o evento (empresas citadas como assessoras, credoras ou vítimas de terceiros são removidas da atribuição). Requer o secret `GEMINI_API_KEY` no GitHub Actions (Settings → Secrets → Actions); sem ele, segue só com keywords (fail-open). Só artigos novos são validados.
