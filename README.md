[README.md](https://github.com/user-attachments/files/30442068/README.md)
# Radar de Risco — Monitor de Notícias de Emissores

## Cobertura oficial × cobertura ampla (Fase 4A)

Todo emissor da watchlist tem, no mínimo, **cobertura ampla**: Google News/RSS,
imprensa geral e fontes internacionais. A **cobertura oficial/reforçada** é um
acréscimo — RI, regulador, sistema de filings — reservada a quem tem fonte
direta disponível. Ausência de RI **não** é ausência de cobertura.

O campo `coverage` de cada emissor (no payload e na exportação) assume:

| Valor | Significado |
|---|---|
| `oficial/reforçada` | coletado diretamente de SEC/EDGAR, CVM/IPE ou RI cadastrado |
| `ampla (oficial condicional à CVM)` | não listada brasileira — depende de ser companhia registrada; `--audit-cvm` confirma |
| `ampla` | monitorado por notícias e fontes públicas |

**Pontos que não podem ser confundidos:**

- **Cobertura ampla ≠ cobertura oficial/reforçada.** Todo emissor tem cobertura
  ampla (Google News/RSS, imprensa, fontes públicas) — é o **piso**, nunca
  ausência. A oficial/reforçada é um **acréscimo** via CVM/IPE, SEC/EDGAR, RI ou
  regulador direto.
- **Não aparecer no IPE não é ausência de cobertura de notícias.** É só ausência
  de cobertura *oficial* por aquele dataset — a ampla continua valendo.
- **O IPE/`cad_cia_aberta` cobre companhias abertas/filiantes CVM.** FIIs e
  fundos **não** são companhias abertas: sua ausência ali não é falha de
  filiação (`nao_aplicavel_dataset_ipe`), e a cobertura oficial deles exige o
  **módulo de fundos estruturados** da CVM (a implementar).
- **Alias curto não confirma filiação sozinho**; múltiplos candidatos entram em
  `revisar`.

**Ordem de prioridade do casamento (forte → fraco):** código CVM → CNPJ → razão
social → ticker/listagem (apoio para listadas) → nome exato → alias longo. Para
emissores listados grandes, prefira sempre o identificador forte.

### Auditoria CVM (`--audit-cvm`)
O IPE/`cia_aberta` cobre **companhias abertas** (listadas e não listadas
registradas na CVM). A auditoria cruza a watchlist brasileira com todos os
protocolos do ano e responde, com dado, quantos emissores são de fato
**filiantes**.

**Casamento por identificador forte (Fase 4B.1).** O match antigo por
substring de nome/alias gerava falso positivo — *Vale* casava com *Vale Bonito
Agropecuária S/A*. Agora o casamento é por camadas, da mais forte à mais fraca,
e **termo curto (< 6 chars) nunca confirma sozinho**:

1. **CNPJ / código CVM** declarados no cadastro (`cnpj`, `codigo_cvm`);
2. **CNPJ / código CVM auto-resolvidos** do cadastro oficial `cad_cia_aberta`
   (código CVM ↔ CNPJ ↔ razão social) — único candidato aplica; ambíguo vai para revisão;
3. **razão social oficial** idêntica;
4. **nome do emissor** idêntico à companhia no IPE (igualdade, não substring);
5. **termo de nome/alias** por palavra inteira dentro de razão social maior.

Quando há **mais de uma companhia possível**, ou o único casamento veio de
termo curto, o emissor entra em **`revisar`** (não é contado como filiante). O
CSV registra, por linha, a **companhia efetivamente casada**, CNPJ e código CVM
casados, o **tipo de match**, a confiança, os candidatos e o **motivo da
decisão** — para rastreabilidade.

Status possíveis: `filiante_cvm`, `nao_filiante`,
`esperado_filiante_sem_protocolo_no_ano` (listada registrada sem protocolo no
ano), `revisar`, `nao_aplicavel_veiculo` (gestora) e
`nao_aplicavel_dataset_ipe` (FII/fundo — ver abaixo). Saída:
`auditoria_cobertura_cvm.csv`.

**FIIs e fundos não são companhias abertas.** Não constam no dataset
`cia_aberta` e por isso recebem `nao_aplicavel_dataset_ipe`, **não**
`esperado_filiante_sem_protocolo` — a ausência aqui não é falha de filiação, é
dataset errado. A cobertura oficial de fundos exige o dataset de **fundos
estruturados** da CVM (informes/eventos), ainda **a implementar**.

### Probe de fontes oficiais (`--probe-sources`)
Mede — sem coletar nada — a acessibilidade de cada URL de `official_sources`:
status HTTP, tipo de conteúdo, tamanho, presença de RSS declarado, indício de
renderização por JavaScript, sinal de anti-bot e latência. Cada URL recebe um
veredito operacional (*implementar agora* / *preparar mas desativado* /
*anti-bot* / *indisponível*).

Existe porque o ambiente de desenvolvimento não alcança os portais dos
reguladores latino-americanos, e escrever scraper às cegas produziria
exatamente o coletor frágil que se quer evitar. O probe distingue **bloqueio do
ambiente de execução** de **anti-bot da fonte** — um diagnóstico que confunde os
dois seria pior que nenhum. Rode-o no GitHub Actions, onde a rede é aberta.
Saída: `probe_fontes_oficiais.csv`.

### Agências de rating
Sem coletor dedicado, por decisão: S&P, Moody's e Fitch não oferecem RSS/API
pública estável e o conteúdo profundo está atrás de paywall. A cobertura vem da
imprensa, com **peso 0,99** para 14 domínios de agências — incluindo as locais
latino-americanas (Feller Rate, Humphreys, HR Ratings, Verum, BRC, Value & Risk,
Austin, SR Rating). Os termos de rating foram ampliados para capturar
CreditWatch, Rating Watch, *revisión a la baja*, *clasificación de riesgo*,
*bajo observación* e equivalentes em inglês e espanhol.

### Argentina
Documentada como **coberta via EDGAR**: YPF e Telecom Argentina são ADRs na NYSE
e já entram pelo coletor da SEC. O regulador local (CNV/AIF) ficou **fora do
escopo** — é a fonte mais instável do conjunto e o ganho marginal seria zero.
Revisar apenas se entrar emissor argentino relevante sem cobertura via EDGAR.

### Cobertura oficial prioritária
Conceito separado do tier: **representação ≥ 0,3%**, independentemente do tier,
ou relevância qualitativa definida pela área. Existe porque a regra de tier por
materialidade não produz Tier 1 internacional (o maior é a Bunge, com 0,598%) —
e forçar isso distorceria os tiers. Um emissor pode não ser Tier 1 e ainda assim
ter prioridade de fonte oficial.


## Cobertura internacional (Fase 3, jul/2026)

### Fontes oficiais por país
O bloco `official_sources` do config documenta regulador, sistema de filings, bolsa,
idioma e viabilidade técnica de coleta para 14 jurisdições. Implementado:

- **EUA — SEC/EDGAR**: coletor próprio (`fetch_edgar_filings`) que lê
  `data.sec.gov/submissions/CIK##########.json` (endpoint documentado), resolve
  ticker→CIK via `company_tickers.json` com cache em disco, filtra os formulários-gatilho
  (**8-K, 6-K, 10-K, 10-Q, 20-F**) na janela de evolução e entra no pipeline com
  confiança **oficial** — o equivalente americano da CVM/IPE. Respeita o limite de
  10 req/s da SEC (margem de 8) e envia User-Agent identificável, obrigatório.
- **Chile (CMF), México (CNBV/EMISNET), Colômbia (SIMEV), Argentina (CNV/AIF)**:
  cadastrados com URL e viabilidade documentada. Nenhum oferece API pública — a coleta
  exigirá scraping dedicado, previsto para a próxima etapa.

### Imprensa internacional
11 feeds RSS **abertos e verificados** em `custom_feeds`: CNBC (Finance/Economy),
MarketWatch, Yahoo Finance (EUA); El Economista e El Financiero (México); Portafolio
Economía/Empresas (Colômbia); Ámbito Economía/Finanzas e Infobae (Argentina).
Veículos com paywall duro (WSJ, FT, Diario Financiero, Reforma) ficaram de fora
deliberadamente. `source_trust` ganhou 27 domínios oficiais e 42 de imprensa.

### Idioma e tradução
Foi adotada a estratégia **translate-then-classify** como decisão técnica interna:
padroniza a classificação em um único idioma (português, onde já está a taxonomia) e
reduz a perda de eventos vindos de fontes em inglês e espanhol. O pipeline detecta o
idioma (`detect_language`) e traduz **apenas título e resumo**, preservando
`title_original` e o link da matéria.

- A detecção usa, em ordem: campo do coletor → idioma do feed → idioma cadastrado do
  emissor → heurística lexical.
- Tradução em lotes de 20 pelo mesmo Gemini já usado na consolidação.
- **Degradação graciosa:** se não houver chave de API ou a cota estourar, o pipeline
  não quebra e as notícias seguem com o texto original. Para isso funcionar de fato,
  os 15 eventos corporativos mais materiais e os eventos de veículo receberam
  **palavras-chave em inglês e espanhol** (rede de segurança) — uma notícia
  "Ford downgraded by rating agency" é classificada mesmo sem tradução.
- No dashboard, cada card mostra selos de **idioma**, **país do emissor** e
  **"traduzido"**, com o título original em itálico logo abaixo.

## Taxonomia de Gestoras/Fundos

Implementada e ativa. São **11 eventos de risco de veículo/gestor**, ancorados em
SEC Rule 22e-4 (17 CFR 270.22e-4), ESMA sob AIFMD II/UCITS (suspensão, gates, notice
period, side pockets, ADTs, resgate em espécie) e IOSCO (terminação de fundos):

`suspensao_resgates` · `gate_resgates` · `side_pocket` · `liquidacao_fundo` ·
`resgates_elevados` · `key_person` · `restricao_liquidez_fundo` ·
`dificuldade_valoracao` · `perdas_relevantes_fundo` · `investigacao_gestora` ·
`reabertura_resgates` (positivo)

**Separação por natureza do emissor.** Cada evento da taxonomia tem `applies_to`,
listando os grupos em que faz sentido. Eventos corporativos (recuperação judicial,
covenant breach) **não pontuam** Gestoras/Fundos; eventos de veículo não pontuam
companhias. Fraude, intervenção regulatória e suspensão de negociação valem para
ambos. As gestoras saíram do modo `monitoramento_limitado` e passaram a
`taxonomia_propria` — pontuam normalmente, mas só pelo que lhes cabe.


## Classificação cadastral: natureza do emissor ≠ instrumento (jul/2026)

**Regra fundamental.** O subgrupo de um emissor reflete a **natureza da entidade
monitorada** — companhia aberta, empresa fechada, fundo listado ou gestora/veículo.
Ele **nunca** é derivado do instrumento pelo qual a posição aparece na carteira.

- Um `CORPORATE BOND` não torna o emissor "não listado": Ford, GM, Citigroup,
  JPMorgan, CVS, Cemex, LATAM e Ecopetrol são companhias abertas que entram na
  carteira via dívida.
- Uma `AÇÃO (BRL)` não torna o emissor "listado": SPEs florestais, Hospital Care e
  Grupo CVLB são participações privadas registradas como ações ON.

**Campos cadastrais obrigatórios** (todos os 160 emissores, sem inferência):
`asset_class`, `country`, `region`, `language`, `tier`, `aliases`.

**Campos separados por função:**
- `ticker` — cadastral (validação, exportação, conferência de listagem).
- `listing` — bolsa onde negocia (B3, NYSE, Nasdaq, Santiago, BMV, BVC…).
- `aliases` — **apenas** termos de busca. Tickers curtos (`F`, `C`, `GM`, `ET`, `MU`)
  nunca entram como alias, porque casariam com palavras comuns nas manchetes.

**País/região = domicílio do emissor**, não a praça do instrumento. Um BDR negociado
no Brasil de emissor estrangeiro registra o país do emissor. Não há fallback para
"Brasil": emissor sem domicílio aparece como **"A revisar"** e gera warning.

## Gestoras/Fundos: taxonomia própria de veículo/gestor

O subgrupo `gestora_fundo` cobre gestoras, fundos e veículos — risco de **veículo e
gestor**, não risco corporativo operacional. Desde a Fase 3 eles usam
`scoring_mode: taxonomia_propria`:

- pontuam **apenas** pelos 11 eventos de veículo/gestor (resgates, gates, side pocket,
  liquidação, key person risk, restrição de liquidez, investigação regulatória…);
- eventos corporativos **não** pontuam esse grupo, salvo os explicitamente aplicáveis
  (fraude, intervenção regulatória, suspensão de negociação);
- o dashboard exibe aviso explicando a taxonomia, e a exportação traz as colunas
  "Modo de monitoramento" e "Modo de cobertura".

## Cobertura oficial de fundos/FIIs (Fase 4C)

**Por que FIIs/fundos não entram no IPE de companhias.** O dataset
`ipe_cia_aberta`/`cad_cia_aberta` cobre **companhias abertas**. FIIs, FIAGRO,
FIP e FIDC são **fundos**, registrados e regulados por outro arcabouço — não
constam nesse dataset. Por isso a auditoria os marca `nao_aplicavel_dataset_ipe`
(dataset errado), **nunca** como falha de filiação, e a cobertura oficial deles
é `oficial pendente (fundos)` até existir um módulo próprio.

**Taxonomia (grupo por natureza do veículo):** companhias abertas (listadas) e
não listadas registradas → auditadas no IPE; **FII/FIAGRO** (fundos listados,
cota 11) → grupo *FIIs*; **FIP/FIDC** → grupo *Gestoras/Fundos*; gestoras →
*Gestoras/Fundos* com taxonomia própria. O campo `fund_type` registra o tipo; um
diagnóstico sinaliza divergências (ex.: um FIP-IE listado hoje em `fii`) como
pendência — sem reclassificar automaticamente.

**Plano de cobertura oficial de fundos (a implementar, nada ativo):** a fonte
estruturada de eventos de fundos é o **FNET (Fundos.Net)** — sistema da CVM
operado pela B3, onde FII/FIAGRO/FIDC/FIP publicam Fato Relevante, Comunicado ao
Mercado, Aviso aos Cotistas e assembleias. Candidato principal a coletor oficial
(a validar no probe). `dados.cvm.gov.br` (FII/FIDC/FIP) cobre cadastro/informes;
B3 e ANBIMA são secundárias. Tudo cadastrado em `fund_sources` com
`enabled: false` e `probe_targets` para medição no Actions — **sem coletor
ativo**.

**Campos cadastrais de fundo:** `fund_type`, `manager`, e em `official`:
`fund_source`, `fund_cnpj`, `admin`, `cobertura_oficial_status`. Onde o dado não
é conhecido, fica `a_confirmar` (não se inventa CNPJ/identificador).

**Pendente de validação no Actions:** o veredito técnico do FNET/CVM (probe de
`fund_sources`), a decisão de reclassificação do FIP-IE, e o preenchimento de
CNPJ/administrador dos fundos.

Fixtures: `python risk_dashboard.py --test-fund-coverage` (offline) trava as
regras acima (FII fora do IPE, taxonomia FIP/FIAGRO, robustez do
`coverage_summary`, probe de fundos desativado).

## Universo de cobertura: 160 emissores × carteira total (Fase 4D)

O dashboard **não** monitora linha a linha da carteira — ele consolida por
**emissor monitorável**. O funil, do bruto ao que aparece na tela:

1. **Carteira/posições brutas** — todas as posições dos fundos;
2. **Instrumentos** — cada posição é um instrumento (ação, debênture, cota, título…);
3. **Emissores / grupos econômicos** — vários instrumentos colapsam num mesmo emissor;
4. **Emissores monitoráveis** — nem todo instrumento vira emissor noticioso: soberanos, derivativos, caixa, veículos internos e *wrappers* não têm "fato relevante" próprio;
5. **Materialidade/relevância** — corte por representação na carteira;
6. **Watchlist atual (160)** — o universo monitorado hoje;
7. **Cobertura ampla** — piso de **todos** os 160 (Google News/RSS, imprensa, fontes públicas);
8. **Cobertura oficial/reforçada** — reforço via CVM/IPE, SEC/EDGAR, RI ou regulador direto (não é requisito para estar monitorado);
9. **Pendências** — não listadas aguardando audit CVM, fundos aguardando módulo próprio, LatAm aguardando probe.

Pontos-chave: cobertura ampla existe para todos; cobertura oficial é **reforço**,
não requisito; a expansão é **faseada** de propósito, para evitar scraping frágil
e falso positivo. Alguns nomes ficam em módulo futuro (fundos via FNET, LatAm via
re-probe) — documentados, não esquecidos.

## Validação antes do deploy

O workflow roda `--strict-groups`, que **aborta** o deploy se algum emissor estiver sem
`asset_class`/grupo resolvível ou sem `aliases`. Warnings cobrem: campos obrigatórios
ausentes, país/região em fallback, listada sem ticker/bolsa, **não listada com
ticker/bolsa** (assinatura do erro de classificar pelo instrumento), Tier 1 sem fonte
oficial, gestora sem `vehicle_kind`/`scoring_mode`, alias genérico, duplicados e
tickers repetidos entre emissores.

### Testes do casamento CVM (`--test-cvm-fixture`)
Suíte offline (sem rede, sem dados de produção) que trava as regras da Fase
4B.1. Rode localmente:

```
python risk_dashboard.py --test-cvm-fixture
```

Cobre, com índices IPE/cadastro sintéticos: Vale não pode casar com *Vale Bonito
Agropecuária* (id forte); alias curto não confirma sozinho; Grupo CVLB fica em
`revisar` sem id forte; Fisia só casa com razão social/identificador robusto;
Aegea/GSH/Hospital Care casam por identificador; FIIs caem em
`nao_aplicavel_dataset_ipe`; múltiplos candidatos → `revisar`; CNPJ/código CVM
têm precedência sobre substring; listada grande sem id forte → `revisar` (não
chuta); dataset indisponível degrada de forma controlada. Sai com código 0 se
todos passarem.


## Exportação: download do radar e do resumo (jul/2026)

O dashboard permite baixar os dados diretamente, sem backend (tudo gerado no
front-end a partir do payload — funciona no site estático do Render):

- **Baixar radar (CSV)** — na aba "Radar de emissores". Respeita a janela e o
  filtro de grupo atuais. Usa separador `;` e BOM UTF-8 (abre corretamente no
  Excel em português).
- **Baixar radar (Excel .xlsx)** — gerador `.xlsx` nativo embutido (Office Open
  XML escrito em JS puro, sem depender de CDN externo). Três abas: **Ranking**,
  **Eventos por emissor** e **Metadados** (janela, filtro, data/hora, totais).
- **Baixar resumo (.md)** — na aba "O que mudou". Contém data/hora, janela,
  emissores críticos, mudanças de status e sinais novos.

Campos do radar exportado: posição, emissor, subgrupo, país, região, tier,
score, classificação, variação, evento principal, severidade, direção, nº de
eventos, nº de fontes, última notícia, link principal, janela e data/hora de
geração. (País e região saem como "Brasil" enquanto a watchlist for toda
nacional; passam a ser preenchidos quando os campos entrarem no cadastro.)
...
## Botões de subgrupo fixos, derivados do cadastro (jul/2026)

**Regressão corrigida.** Ao introduzir o chip "A revisar" condicional, os botões de subgrupo passaram a ser derivados dos grupos *presentes no `evolution`* (ou seja, só emissores com sinal na janela). Efeito colateral: numa janela de 7 dias sem notícias de FIIs ou de não listadas, esses botões **sumiam**.

**Regra correta (agora implementada).** Os botões de subgrupo vêm do **cadastro completo da watchlist**, não das notícias:
- O pipeline gera `payload.asset_groups` via `build_asset_groups_meta(cfg)` — a lista fixa dos grupos existentes na watchlist, cada um com o total de emissores monitorados (ex.: Empresas listadas 51, Não listadas 14, FIIs 8).
- O frontend renderiza os botões a partir dessa lista, sempre visíveis, independentemente da janela.
- O que muda com a janela é só: os cards exibidos, os contadores de status e o estado vazio. Cada chip mostra `Xcom-sinais/Ytotal` (ex.: `0/14`), com o total no tooltip.
- Selecionar um grupo sem sinais na janela mostra o estado vazio *"Nenhum emissor do grupo '…' possui sinais classificados nesta janela."* — o botão permanece.

## Correção da segmentação por grupo de ativos (jul/2026)

**Sintoma reportado.** Na aba "Radar de emissores", ao filtrar por "Não listadas (PE/Crédito)", apareciam empresas listadas — o caso mais claro era a **Axia Energia** (ex-Eletrobras, listada na B3 sob AXIA3), que surgia como não listada.

**Causa raiz.** O erro não estava no código de filtragem, e sim no **cadastro da watchlist**: a Axia estava com `type: nao_listada`. O mapeamento fazia o que devia (lê o cadastro e classifica); o dado de entrada é que estava errado. Uma auditoria dos 15 emissores marcados como `nao_listada` confirmou que a Axia era o único erro — os outros 14 são de fato empresas privadas/PE/crédito privado.

**O que mudou.**
- A segmentação passou a ter um campo cadastral canônico e explícito, **`asset_class`** (valores `listada`, `nao_listada`, `fii`), resolvido de forma determinística por `asset_group_of_company()`. O campo legado `type` continua aceito para retrocompatibilidade. Emissores sem base cadastral **e** sem ticker de ação vão para **"A revisar"** — nunca são inferidos a partir de notícia, evento, score ou tier, e nunca aparecem como `undefined`/`null`/`NaN`.
- Correções pontuais no cadastro: **Axia Energia** → `listada` (com os tickers AXIA3/AXIA5/AXIA6 nos aliases); **B3**, **MercadoLibre** e **StoneCo** marcadas explicitamente como `listada` (a última via bolsa estrangeira, por isso sem ticker B3).
- Cada emissor pertence a **exatamente um** grupo (determinístico) — validado em teste.

**Validação automática no pipeline.** Ao carregar o config, `validate_asset_classes()` roda e emite:
- **ERRO** se algum emissor ficar sem grupo resolvível (com `--strict-groups`, isso **bloqueia o deploy** até a correção);
- **WARNING** se um emissor tiver ticker de ação mas estiver como não listado (o caso que gerou este bug), ou se estiver como listado sem ticker de ação (revisar — pode ser ADR/BDR).

**Histórico.** O grupo de ativos **não é persistido** no `risk_history.json` nem no snapshot `last_run` (que guardam só artigos, eventos e `{status, score}` por emissor). Ele é derivado do config a cada execução, então a correção do cadastro **propaga sozinha** no próximo run — não é necessária migração de dados.

**Tabela de auditoria.** O arquivo `auditoria_grupos_ativos.csv` lista os 160 emissores com nome, ticker, tipo cadastral, subgrupo atribuído/esperado e status (ok/revisar).
...
## Nota de correção (160 emissores, quota do Gemini)
