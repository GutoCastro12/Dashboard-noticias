...
_CONTEXT_PATTERNS = (
    ...
    r"{A}\b\s*,?\s+(?:e\s+)?(?:credor\w*|assessor\w*|administrador\w*|custodiante|"
    r"coordenador\w*|fiduciari\w*|underwriter\w*|auditor\w*)\b",
    r"\b(?:credor\w*|assessor\w*|administrador\w*|custodiante|coordenador\w*|fiduciari\w*)"
    r"\w*\s+(?:e\s+)?(?:[ao]\s+)?{A}\b",
    ...
    # emissor RESOLVE evento de TERCEIRO ("Vale finaliza recuperação judicial
    # DA SAMARCO"): o evento de crédito é da outra entidade, não do emissor.
    r"{A}\b\s+(?:\w+\s+){0,2}(?:finaliza\w*|conclui\w*|encerra\w*|supera\w*|assume\w*|"
    r"assumiu|adquire\w*|compra\w*|arremata\w*)\s+(?:[ao]s?\s+)?"
    r"(?:recuperac\w+ judicial|falenci\w+|reestruturac\w+|concordata|insolvenci\w+)"
    r"\s+d[aeo]s?\s+\w+",
)
...
def run_attribution_tests(cfg: dict | None = None) -> int:
    ...
    casos = [
        ...
        ("Vale finaliza recuperação judicial da Samarco e reafirma compromisso",
         ["Samarco Mineração"], "RJ é da Samarco; Vale só resolve"),
        # controles positivos
