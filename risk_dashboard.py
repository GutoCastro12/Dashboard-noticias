...
def scrape_ri_page(company: str, url: str, cfg: dict,
    ...
    try:
        ...
        print(f"   ⚠️  RI de {company} indisponível: {exc}")
        return [], "erro"
...
def _gemini_call(model, prompt: str, sleep_s: float):
    """Chamada com backoff simples para o rate limit do free tier."""
    for attempt in (1, 2):
        try:
            ...
            if attempt == 1 and ("429" in str(exc) or "quota" in str(exc).lower()):
                print("   ⏳ Rate limit do Gemini — aguardando 30s…")
                time.sleep(30)
                continue
            raise
...
def consolidate_with_llm(articles: list[dict], cfg: dict, history: dict) -> list[dict]:
    ...
    model = genai.GenerativeModel(llm.get("model", "gemini-2.5-flash"))
    ...
    for comp, arts in by_company.items():
        ...
        try:
            ...
            analises = {int(x["i"]): x for x in data.get("analises", [])
                        if isinstance(x.get("i"), (int, float, str)) and str(x["i"]).isdigit()
