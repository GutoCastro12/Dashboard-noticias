...
# Mapa do tipo do emissor (na watchlist) para o grupo de ativos exibido no
# filtro do dashboard. A fonte é o campo `type` da configuração — não há
# classificação hardcoded no frontend. `nao_listada` cobre tanto Private
# Equity quanto Crédito Privado (a watchlist não distingue os dois).
ASSET_GROUP_BY_TYPE = {
    "empresa": "listed_companies",
...
def asset_group_of(ctype: str | None) -> str:
    """Grupo de ativos normalizado a partir do tipo do emissor. Emissores sem
    tipo reconhecido caem em 'nao_classificado' (nunca undefined/null)."""
    return ASSET_GROUP_BY_TYPE.get(ctype or "", "nao_classificado")
...
def build_evolution(history: dict, cfg: dict, window_days: int | None = None,
    ...
    for company, occurrences in per_company.items():
        ...
        rows.append({
            ...
            "tier": m["tier"],
            "type": m["type"],
            "asset_group": asset_group_of(m["type"]),
            "book_pct": book_pct,
            "status": status,
