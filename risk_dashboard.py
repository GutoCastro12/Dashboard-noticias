...
def resolve_google_news_urls(articles: list[dict], history: dict, cfg: dict) -> None:
    ...
    to_resolve = [a for a in articles
                  if "news.google.com" in (a.get("url") or "")
                  and a["url"] not in resolved_cache]
    if to_resolve:
    ...
    for art in to_resolve:
        ...
        resolved_cache[gurl] = real or ""
        if real:
            n_ok += 1
        time.sleep(0.4)  # gentil com o Google (evita 429)
    # aplica o cache a todos (inclusive os já resolvidos em execuções passadas)
    for art in articles:
        u = art.get("url") or ""
        if "news.google.com" in u and resolved_cache.get(u):
            art["url"] = resolved_cache[u]
    if to_resolve:
        print(f"   ✅ {n_ok}/{len(to_resolve)} resolvidos para o link direto.")
    # limpa cache antigo (mantém <= 3000 entradas)
...
def _resolve_one_gnews(gurl: str, domain: str, session: requests.Session) -> str | None:
    ...
    try:
        ...
        pass
    # 2) segue o redirect HTTP e lê a URL final
    try:
        r = session.get(gurl, timeout=15, allow_redirects=True)
        final = r.url
        if final and "google.com" not in final:
            return final
        # alguns retornam o link num meta/refresh ou data-url no corpo
        ...
        pass
    # 3) fallback: home do veículo (melhor que link quebrado)
...
def main():
    ...
    # Resolve os redirects do Google News (news.google.com/rss/articles/CBMi…)
    # para o link direto do veículo — SÓ nos artigos que entram no dashboard
    # (dezenas, não milhares), e só os ainda não resolvidos no histórico.
