...
def _resolve_corroboration_urls(history: dict, session, budget: int = 40) -> None:
    """[fix de links] Resolve também as URLs das fontes CORROBORADORAS.

    A versão anterior de `resolve_history_urls` só percorria as chaves de
    history["articles"] — as fontes principais. As corroborações mantinham o
    token `news.google.com/rss/articles/...`, que a rede corporativa recusa.
    Medição no histórico real antes da correção: 39,0% das principais contra
    98,6% das corroboradoras ainda com redirecionador."""
    try:
        import link_debt_audit as _lk
    except Exception:
        return
