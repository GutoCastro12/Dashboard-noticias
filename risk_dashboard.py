...
_STOCK_TICKER_RE = re.compile(r"^[A-Z]{4}\d{1,2}$")


def has_stock_ticker(company: dict) -> str | None:
    """Retorna o primeiro ticker de AÇÃO (padrão B3: 4 letras + 1-2 dígitos)
    encontrado nos aliases, ou None. Tickers de fundo (final 11) contam como
    ticker de negociação, mas a distinção fii/ação é feita pelo asset_class."""
    for a in company.get("aliases", []) or []:
        if _STOCK_TICKER_RE.fullmatch((a or "").strip()):
            return a.strip()
    return None
...
def validate_asset_classes(watchlist: list[dict]) -> list[str]:
    """Valida a segmentação cadastral de toda a watchlist. Retorna a lista de
    mensagens de erro/warning. Regras:
      - emissor sem grupo resolvível → ERRO (bloqueia o deploy);
      - emissor com ticker de ação classificado como não listado → WARNING
        (provável erro de cadastro, como foi o caso da Axia Energia);
      - emissor classificado como listada sem ticker de ação → WARNING leve
        (pode ser ADR/BDR/estrangeira; apenas revisar)."""
    msgs: list[str] = []
    for c in watchlist:
        ...
        if grp == "a_revisar":
            msgs.append(f"ERRO: emissor '{name}' sem grupo de ativos cadastral "
                        f"definido (asset_class/type ausente ou inválido).")
        if tk and grp == "nao_listada":
            msgs.append(f"WARNING: emissor '{name}' tem ticker de ação ({tk}) "
                        f"mas está classificado como 'Não listadas'. Verifique o cadastro.")
        if grp == "listed_companies" and not tk and not c.get("official"):
            msgs.append(f"WARNING: emissor '{name}' está como 'Empresas listadas' "
                        f"mas não tem ticker de ação nos aliases. Confirme a listagem.")
    return msgs
