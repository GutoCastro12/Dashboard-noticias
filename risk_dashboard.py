...
def audit_international_coverage(cfg: dict, history_path: str = "risk_history.json",
    ...
    for c in est:
        ...
        ri_news = bool(off.get("news") or off.get("ri"))
        ...
        if not pesquisado:
            motivo, status = "emissor NÃO pesquisado (fetch_every_n_runs=0)", "BLOQUEIO"
        ...
        linhas.append({
            ...
            "pesquisado_na_rotacao": "sim" if pesquisado else "NÃO",
            "locale_usado": f"{hl}/{gl}", "aliases_usados": "; ".join(str(a) for a in aliases),
            "exposicao_aprox": exp,
            "midia_local_ativa": "sim" if pesquisado else "não",
            "noticias_7d": n7, "noticias_30d": n30, "noticias_90d": n90,
            "ultima_noticia": ultima,
            "sec_edgar_ativo": "sim" if edgar else "não",
    ...
    for l in linhas:
        p = pais[l["pais"]]
        p["monitorados"] += 1
        p["pesquisados"] += (l["pesquisado_na_rotacao"] == "sim")
        ...
        p["zero_resultados"] += (l["noticias_90d"] == 0 and l["pesquisado_na_rotacao"] == "sim")
    ...
    _write_rows_csv(os.path.join(outdir, "cobertura_internacional_por_pais.csv"), rows_pais,
                    ["pais", "monitorados", "pesquisados", "com_noticia_90d",
    ...
    tot = len(linhas)
    nao_pesq = sum(1 for l in linhas if l["pesquisado_na_rotacao"] != "sim")
    ...
    print(f"   4H.1: {tot} estrangeiros · {nao_pesq} NÃO pesquisados · {com_not} com notícia 90d "
          f"· {of_ativa} fonte oficial ativa · {so_doc} só documentada")
    return {"total": tot, "nao_pesquisados": nao_pesq, "com_noticia_90d": com_not,
...
def main():
    ...
    if _e4:
        ...
        if args.audit_international_coverage:
            audit_international_coverage(cfg4, args.history, args.exposure_base, args.outdir)
